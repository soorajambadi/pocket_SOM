...
class Node:
    ...
    def __init__(self, FV_size=300, PV_size=300, Y=0, X=0):
...
class SOM:
    ...
    def __init__(self, height=10, width=10, FV_size=300, PV_size=300, radius=False, learning_rate=0.01):
        self.height=height
        self.width=width
    ...
    def train(self, iterations=200, train_vector=[[[0.0],[0.0]]]):
        ...
        for i in range(1,iterations+1):
            ...
            for j in range(len(train_vector)):
                ...
                best=self.best_match(input_FV[0])
                stack=[]
                if i==iterations:
                    self.nodes[best].content.append(j)
                for k in range(self.total):
                    ...
                    if dist < radius_decaying:
                        ...
                        for l in range(self.FV_size):
                            #Learning
                            temp_FV[l]=self.nodes[k].FV[l]+influence*learning_rate_decaying*(input_FV[l][0]-self.nodes[k].FV[l])
    ...
    def predict(self, FV=[0.0]):
        ...
        return self.nodes[best].FV

    def getSimilarArticles(self, FV=[0.0]):
        best=self.best_match(FV)
        return self.nodes[best].content
